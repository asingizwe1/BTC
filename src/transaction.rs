...
// //modifications to make the json look better
// #[derive(Debug, Serialize)] //as long as all types that belong to it can be serialized
// use serde::{Serialize, Serializer};
// use std::fmt;
// //use std::io::{BufRead, Error as ioError};
// //we romved the above coz we are going to create custom errors of our own
// pub enum Error {
//     Io(std::io::Error),
// }
...
//     fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
//         match *self {
//             //ref e will prevent the match from consuming the error message
//             Error::Io(ref e) => write!(f, "IO error: {}", e),
//             Error::UnsupportedSegwitFlag(swflag) => {
//                 write!(f, "unsupported segwit version: {}", swflag)
//             }
...
//     }
// }
// //function sin main are expected to return an error that implements error trait
// impl std::error::Error for Error //implementing custome Error for error
// {
// }
...
// #[derive(Debug)] //manually implement Serialize so that we can set custom fireld names
// pub struct Transaction {
//     //we remove tx id coz for segwit transactions we dont hash all the fields
...
//         Txid(bytes)
//     }
//     //logic from hash_ra_transaction in main.rs
//     fn from_raw_transaction(tx: Vec<u8>) -> Txid {
//         let mut hasher = Sha256::new();
//         hasher.update(&tx);
//         let hash1 = hasher.finalize();
...
// //what ever type that implements Decodable must have size known at compile time
// pub trait Decodable: Sized {
//     //sized trait bound to show that the type to implement this is going to be a sized type
//     fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error>;
// }
// impl Decodable for Witness {
//     fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
...
// impl Decodable for u8 {
//     fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
//         let mut buffer = [0; 1]; //we read 4 bytes into a buffer and return a u32 wrapped in a result
//         r.read_exact(&mut buffer).map_err(Error::Io)?; //map_err will help us convert into our custom error
//         Ok(buffer[0])
//     }
// }
...
//                     let mut inputs = Vec::<TxIn>::consensus_decode(r)?;
//                     let outputs = Vec::<TxOut>::consensus_decode(r)?;
//                     //recorded decoded witness fields that come after output
//                     for txin in inputs.iter_mut() {
//                         //convert collection into interable type
//                         txin.witness = Witness::consensus_decode(r)?; //witness is data required to unlock
//                     }
//                     if !inputs.is_empty() && inputs.iter().all(|input| input.witness.is_empty()) {
//                         Err(Error::ParseFailed(
//                             "witness flag set but no witnesses present",
...
// //encodable trait that will convert tx component struct type back to its raw bytes
...
// impl Encodable for u16 {
//     fn consensus_encode<W: Write>(&self, w: &mut W) -> Result<usize, Error> {
//         let b = self.to_le_bytes(); //convert into u32 into bytes
//         let len = w.write(b.as_slice()).map_err(Error::Io)?; //write returns bytes wrapped in a result
//         Ok(len)
//     }
// }
