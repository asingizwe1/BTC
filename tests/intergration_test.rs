// use std::fs; //this helps you read contents of test json file
// #[test]

// fn test_json() {
//     let raw_transaction = "01000000024d5c1d6f7308bbe95c0f6e1301dd73a8da77d2155b0773bc29"; // let transaction_hex = "01000000024d5c1d6f7308bbe95c0f6e1301dd73a8da77d2155b0773bc29";
//     let json = transaction_decoder::decode(raw_transaction_hex);
//     //expected response is what we read from our test file
//     let expected = fs::read_to_string("tests/test_transaction.json").unwrap();
//     assert_eq!(expected, json);
// }
// //we take inspiration from rust bitcoin library to make our tx's seqwit
// //Decodable for transaction ar https://docs.rs/bitcoin/latest/src/bitcoin/blockdata/transaction.rs.html#1239
use std::fs;

#[test]
fn test_legacy() {
    let raw_transaction_hex = "";
    let json = transaction_decoder::decode(raw_transaction_hex.to_string()).unwrap();
    let expected = fs::read_to_string("tests/test_legacy_transaction.json").unwrap();
    assert_eq!(expected, json);
}

#[test]
fn test_segwit() {
    let raw_transaction_hex = "";
    let json = transaction_decoder::decode(raw_transaction_hex.to_string()).unwrap();
    let expected = fs::read_to_string("tests/test_segwit_transaction.json").unwrap();
    assert_eq!(expected, json);
}
